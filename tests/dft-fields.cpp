...
double compare_array_to_dataset(cdouble *field_array, int array_rank, size_t *array_dims,
  ...
  meep::realnum *rdata = f.read(dataname, &file_rank, file_dims, 2);
  snprintf(dataname, 100, "%s.i", name);
  meep::realnum *idata = f.read(dataname, &file_rank, file_dims, 2);
...
double compare_complex_hdf5_datasets(const char *file1, const char *name1, const char *file2,
  ...
  meep::realnum *rdata1 = f1.read(dataname, &rank1, dims1, expected_rank);
  snprintf(dataname, 100, "%s.i", name1);
  meep::realnum *idata1 = f1.read(dataname, &rank1, dims1, expected_rank);
  ...
  meep::realnum *rdata2 = f2.read(dataname, &rank2, dims2, expected_rank);
  snprintf(dataname, 100, "%s.i", name2);
  meep::realnum *idata2 = f2.read(dataname, &rank2, dims2, expected_rank);
